...
const calcBonus=(tar, ach)=>{
  console.log(tar, ach);
    ...
    if(perc>125){
     ...
     bonus = Math.round((10000+(0.05*0.25*tar)+((ach-1.25*tar)*0.075))*100)/100;
     return bonus;
    }
    if(perc>100){
      bonus = Math.round((10000+((ach-tar)*0.05))*100)/100;
      return bonus;
        }
  if(perc===100){
          bonus = 10000;
  ...
  <table className={classes.table}>
    ...
    <tbody>
      {employees.length > 0 ? (
        ...
        <td>$ { Math.round((emp.achievedTarget/(monthN+1)*12)*100)/100}</td>
